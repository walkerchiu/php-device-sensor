...
class DeviceRegisterFormRequest extends FormRequest
{
    ...
    {
        $rules = [
            'device_id'   => ['required','string','exists:'.config('wk-core.table.device-sensor.devices').',id'],
            'serial'      => '',
        ...
        if (
            ...
            $rules = array_merge($rules, ['id' => ['required','string','exists:'.config('wk-core.table.device-sensor.devices_registers').',id']]);
        } elseif ($request->isMethod('post')) {
            $rules = array_merge($rules, ['id' => ['nullable','string','exists:'.config('wk-core.table.device-sensor.devices_registers').',id']]);
        }
    ...
    public function messages()
    {
        return [
            ...
            'device_id.string'    => trans('php-core::validation.string'),
